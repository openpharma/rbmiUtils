---
title: "Storing and then analysing with rbmiUtils"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{analyse2}
...
This vignette demonstrates how to perform multiple imputation followed by two types of analyses:

- A standard ANCOVA on a continuous endpoint (CHG).
- A `responder` analysis on a binary endpoint (CRIT1FLN).
- We use the {rbmiUtils} package, which extends the capabilities of {rbmi} by providing convenient helpers such as get_imputed_data() and analyse_mi_data(), to store, modify the imputed data, and then analyse later. 


## The statistical analysis plan

Rubin’s Rules after standardization. 

Following multiple imputation, we would fit 1000 logistic regression models and for each, obtain the marginal estimates using beeca. We would then pool these 1000 beeca outputs using Rubin’s Rules.
